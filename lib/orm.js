    ...
    Model.find = function (id, callback) {
        ...
        redis.hgetall(model_name.toLowerCase() + ':' + id, function (err, hash) {
            ...
            if (!err) {
                ...
                if (typeof callback == 'function') callback.call(found ? obj: null, found ? null : true, obj);
            } else {
...
exports.apply_to_models = function (path) {
    var fs = require('fs'), exps = {}, exportz = {};

    fs.readdirSync(path).forEach(function (f) {
        exps[f] = require(path + f);
    ...
    for (var e in exps) {
        for (var c in exportz) {
            exps[e][c] = exportz[c];
        }
    }

    exports.mix_persistence_methods(exportz);
